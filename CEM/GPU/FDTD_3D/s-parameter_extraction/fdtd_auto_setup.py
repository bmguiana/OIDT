...
buffer_xhat = int( usr.nx_clad_wl * wl_clad / delta_x )        # Buffer size, x-direction (cells)
...
nx_swg = int( usr.sgl_wg_height / delta_x )                    # Single waveguide size, x-direction (cells)
ny_swg = int( usr.sgl_wg_width / delta_x )                     # Single waveguide size, y-direction (cells)
nz_swg = int( usr.sgl_wg_length / delta_x )                    # Single waveguide size, z-direction (cells)
pp = int( usr.pitch / delta_x )                                # Pitch between waveguides (cells)

bx = usr.num_cpml + buffer_xhat                                # Border size, x-direction (cells)
by = usr.num_cpml + buffer_yhat                                # Border size, y-direction (cells)
bz = usr.num_cpml + usr.buffer_zhat                            # Border size, z-direction (cells)
nx = 2 * bx + ( usr.num_lines - 1 ) * pp + nx_swg              # Computational domain size, x-direction (cells)
...
if buffer_xhat < pp//2:
    raise Exception('Boundary conditions too small for WG pitch setting. Reduce pitch or increase nx_clad_wl')
...
p1z = sz + int( usr.port1_length / delta_x )
p2z = sz + nz_swg - int( usr.port2_length / delta_x )
if p2z <= p1z:
