...
if usr.sim_type == 's-param':
    ...
    if usr.rough_toggle:
        if kind == 'i':
            ...
            FG_REG = funcs.generate_fg_mask(cfg.nx, cfg.ny, cfg.bx, cfg.by, cfg.ny_swg, cfg.pp, usr.num_lines, cfg.p1x, cfg.nx-cfg.p2x, cfg.rstd, cfg.racl, cfg.delta_x, mode='smooth')
        elif line == 0 and direction == 'f' and kind == 'r':
            # Make new roughness profiles during first line, first reflected sim
            FG_REG = funcs.generate_fg_mask(cfg.nx, cfg.ny, cfg.bx, cfg.by, cfg.ny_swg, cfg.pp, usr.num_lines, cfg.p1x, cfg.nx-cfg.p2x, cfg.rstd, cfg.racl, cfg.delta_x, upper_name=cfg.up, lower_name=cfg.lp, atol=cfg.tol_acl, stol=cfg.tol_std, mtol=0.01)
        else:
            # Load the roughness mask for all other sim types. This conditional hierarchy should make these only during reflected sims
            FG_REG = funcs.generate_fg_mask(cfg.nx, cfg.ny, cfg.bx, cfg.by, cfg.ny_swg, cfg.pp, usr.num_lines, cfg.p1x, cfg.nx-cfg.p2x, cfg.rstd, cfg.racl, cfg.delta_x, mode='load', upper_name=cfg.up+'.npy', lower_name=cfg.lp+'.npy', atol=cfg.tol_acl, stol=cfg.tol_std, mtol=0.01)
    else:
