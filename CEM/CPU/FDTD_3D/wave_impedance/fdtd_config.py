...
# Geometry
sgl_wg_length = 2.0e-6          # Single waveguide length (m, float)
sgl_wg_width = 500e-9           # Single waveguide width (m, float)
sgl_wg_height = 200e-9          # Single waveguide height (m, float)
port_length = 0.5e-6            # Size of settling range along length, subtractive (m, float)

# Roughness
rough_std = 0.0e-9              # Roughness standard deviation (m, float)
rough_acl = 0.0e-9              # Roughness autocorrelation length (m, float)
...
status_readout_interval = 10    # Number of time steps between feedback readouts (time steps, int)
                                # Feedback includes:
...
num_flights = 2.2              # Number of flight times to simulate (, float)
...
# Boundary conditions
num_cpml = 40                   # CPML layer size around simulation space (cells, int)
buffer_zhat = 5                 # Buffer between waveguide[s] and cpml region size, z-direction (cells, int)

# Get gamma value
k0 = 2*np.pi*f0*np.sqrt(mu*eps/eps_rel_bg)          # Free-space wave number (rad/m)
neff = find_neff(np.sqrt(eps_rel_fg*eps_rel_bg), np.sqrt(eps_rel_bg), sgl_wg_height/2, k0, mode='te')
beta = neff*k0                                      # Waveguide wave number (rad/m)
gamma = np.sqrt(beta**2 - eps_rel_bg*k0**2)         # Effective half-width is d+1/gamma
buffer_yhat = int((4/gamma)/delta_x)                # Add appropriate buffer along y (cells)

neff = find_neff(np.sqrt(eps_rel_fg*eps_rel_bg), np.sqrt(eps_rel_bg), sgl_wg_width/2, k0, mode='te')
beta = neff*k0                                      # Waveguide wave number (rad/m)
gamma = np.sqrt(beta**2 - eps_rel_bg*k0**2)         # Effective half-width is d+1/gamma
buffer_xhat = int((4/gamma)/delta_x)                # Add appropriate buffer along y (cells)

# Spatial cell conversion
cpml_range = num_cpml + 1                           # Total CPML layers, including PEC boundary (cells)
nx_swg = int( sgl_wg_width / delta_x )              # Single waveguide size, x-direction (cells)
ny_swg = int( sgl_wg_height / delta_x )             # Single waveguide size, y-direction (cells)
...
p1z = sx + int(port_length/delta_x)
p2z = -1*p1z
