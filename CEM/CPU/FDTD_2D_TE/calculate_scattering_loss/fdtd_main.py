...
if rough_toggle:
    if sim == 2:
        FG_REG = gen_fg_mask_sgl(nx, ny, by, ny_swg, p1x, rstd, racl, delta_x, roughness_profile, mode='gen', atol=0.10, stol=0.10, mtol=0.01)
    elif sim == 1 or sim == 3:
        FG_REG = gen_fg_mask_sgl(nx, ny, by, ny_swg, p1x, rstd, racl, delta_x, roughness_profile, mode='smooth', atol=0.10, stol=0.10, mtol=0.01)
    elif sim == 4:
        FG_REG = gen_fg_mask_sgl(nx, ny, by, ny_swg, p1x, rstd, racl, delta_x, roughness_profile, mode='./rough_profiles/profile'+roughness_profile+'.npy', atol=0.10, stol=0.10, mtol=0.01)
else:
    FG_REG = gen_fg_mask_sgl(nx, ny, by, ny_swg, p1x, rstd, racl, delta_x, roughness_profile, mode='smooth', atol=0.10, stol=0.10, mtol=0.01)
...
HZ_P1 = np.zeros([nt, ny])
HZ_P2 = np.zeros([nt, ny])
...
np.save('./'+output_dir+'/'+hz_port1_name+roughness_profile, HZ_P1)
np.save('./'+output_dir+'/'+hz_port2_name+roughness_profile, HZ_P2)
