...
sim = 1                         # Choose {1, 2, 3, 4}, Select 2 for singular sim
...
# Geometry
sgl_wg_length = 20e-6           # Single waveguide length (m, float)
sgl_wg_width = 200e-9           # Single waveguide width (m, float)
port_length = 5e-6              # Port length for port 1 and 2
...
# # Output files
if sim == 1:
    output_ez1 = 'ez_p1i_p2i0'
    output_ez2 = 'hold'
elif sim == 2:
    output_ez1 = 'ez_p1t_p2i0'
    output_ez2 = 'ez_p2r_p2i0'
elif sim == 3:
    output_ez1 = 'hold'
    output_ez2 = 'ez_p2i_p1i0'
elif sim == 4:
    output_ez1 = 'ez_p1r_p1i0'
    output_ez2 = 'ez_p2t_p1i0'
...
gauss_pulse_deg = -3            # Frequency domain signal strength at f0, must be negative; may be safely ignored when not using st 1 (dB, float)
...
# Array setup
GP = np.zeros(nt)       # Gaussian pulse array
WP = np.zeros(nt)       # Wave packet array
TH = np.zeros(nt)       # Time harmonic signal array
RAMP = np.zeros(nt)     # Auxiliary array for numeric integration of GP
