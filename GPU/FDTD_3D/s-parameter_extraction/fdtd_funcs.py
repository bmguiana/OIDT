...
import numpy as np
from numba import cuda, njit, prange, f8, f4, i4, void
...
@cuda.jit(func_or_sig=void(fa[:,:,:], fa[:,:,:], fa[:,:,:], fa[:], fa[:], fa[:,:,:], i4, i4, i4, i4, i4, i4))
...
#                          Field    , Sc, sx, sy, sz, sx, sy, sz
@cuda.jit(func_or_sig=void(fa[:,:,:], fa, i4, i4, i4, i4, i4, i4))
def update_source(Field, Source, sxb, syb, szb, sxe, sye, sze):
    i, j, k = cuda.grid(3)
    if i >= sxb and i < sxe:
        if j >= syb and j < sye:
            if k >= szb and k < sze:
                Field[i, j, k] = Field[i, j, k] - Source
...
def generate_fg_mask(num_x, num_y, num_z, bord_x, bord_y, wg_height, wg_width, pitch, num_lines, settling_range, end_range, rough_std, rough_acl, dx, mode='gen', out_dir='rough_profiles', correlation=3, upper_name='', lower_name='', atol=0.1, stol=0.1, mtol=0.01):
    ...
    apply_mask(fg_mask, num_x, num_y, num_z, bord_x, bord_y, wg_height, wg_width, pitch, num_lines, var_wrt_len_lower, var_wrt_len_upper)
    return fg_mask


@njit(parallel=True)
def apply_mask(fg_mask, num_x, num_y, num_z, bord_x, bord_y, wg_height, wg_width, pitch, num_lines, var_wrt_len_lower, var_wrt_len_upper):
    for l in range(num_lines):
        offset = l * pitch
        for k in prange(num_z):
            for j in prange(num_y):
                for i in prange(num_x):
                    if j >= (bord_y) and j < (bord_y + wg_width) and i >= (bord_x + offset + var_wrt_len_lower[k]) and i < (bord_x + offset + wg_height + var_wrt_len_upper[k]):
                        fg_mask[i, j, k] = True


#              Exz      , Eyz      , Ezz      , Ex       , Ey       , Ez       , st, kc, ie, je
@cuda.jit(void(fa[:,:,:], fa[:,:,:], fa[:,:,:], fa[:,:,:], fa[:,:,:], fa[:,:,:], i4, i4, i4, i4))
def map_efield_plane_td(Exz, Eyz, Ezz, Ex, Ey, Ez, step, kcut, ie, je):
    i, j = cuda.grid(2)
    if i < ie:
        if j < je:
            Exz[step, i, j] = 0.5 * (Ex[i, j, kcut] + Ex[i+1, j, kcut])
            Eyz[step, i, j] = 0.5 * (Ey[i, j, kcut] + Ey[i, j+1, kcut])
            Ezz[step, i, j] = 0.5 * (Ez[i, j, kcut] + Ez[i, j, kcut+1])


#              Hxz      , Hyz      , Hzz      , Hx       , Hy       , Hz       , st, ct, ie, je
@cuda.jit(void(fa[:,:,:], fa[:,:,:], fa[:,:,:], fa[:,:,:], fa[:,:,:], fa[:,:,:], i4, i4, i4, i4))
def map_hfield_plane_td(Hxz, Hyz, Hzz, Hx, Hy, Hz, step, kcut, ie, je):
    i, j = cuda.grid(2)
    if i < ie:
        if j < je:
