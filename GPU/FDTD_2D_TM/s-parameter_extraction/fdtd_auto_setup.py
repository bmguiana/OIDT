...
nx_swg = int( usr['sgl_wg_length'] / delta_x )                    # Single waveguide size, x-direction (cells)
ny_swg = int( usr['sgl_wg_height'] / delta_x )                    # Single waveguide size, y-direction (cells)
...
if usr['source_type'] == 1:
    ...
    raise Exception('Pick a valid source type')
...
if usr['source_type'] == 1:
    J_SRC = usr['source_amp'] * GP
elif usr['source_type'] == 2:
    J_SRC = usr['source_amp'] * WP
elif usr['source_type'] == 3:
    J_SRC = usr['source_amp'] * RAMP * TH
