...
#              Eyz    , Ezz    , Ey       , Ez       , zs, jc, kc, ie
@cuda.jit(void(f4[:,:], f4[:,:], f4[:,:,:], f4[:,:,:], i4, i4, i4, i4))
def map_efield_zwave(Eyz, Ezz, Ey, Ez, zwstep, jcut, kcut, ie):
    i = cuda.grid(1)
    if i < ie:
        Eyz[zwstep, i] = 0.5 * (Ey[i, jcut, kcut] + Ey[i, jcut+1, kcut])
        Ezz[zwstep, i] = 0.5 * (Ez[i, jcut, kcut] + Ez[i, jcut, kcut+1])


#              Hyz    , Hzz    , Hy       , Hz       , zs, jc, kc, ie
@cuda.jit(void(f4[:,:], f4[:,:], f4[:,:,:], f4[:,:,:], i4, i4, i4, i4))
def map_hfield_zwave(Hyz, Hzz, Hy, Hz, zwstep, jcut, kcut, ie):
    i = cuda.grid(1)
    if i < ie:
        Hyz[zwstep, i] = 0.25 * (Hy[i, jcut, kcut] + Hy[i+1, jcut, kcut] + Hy[i, jcut, kcut+1] + Hy[i+1, jcut, kcut+1])
        Hzz[zwstep, i] = 0.25 * (Hz[i, jcut, kcut] + Hz[i+1, jcut, kcut] + Hz[i, jcut+1, kcut] + Hz[i+1, jcut+1, kcut])
